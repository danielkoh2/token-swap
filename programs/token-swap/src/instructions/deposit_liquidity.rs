...
use crate::{constants::*, state::Pool, error::TutorialError};
...
pub fn deposit_liquidity(ctx: Context<DepositLiquidity>, amount_a: u64, amount_b: u64) -> Result<()> {
    ...
    (amount_a, amount_b) = if pool_creation {
        ...
        if pool_a.amount > pool_b.amount {
            (I64F64::from_num(amount_b).checked_mul(ratio).unwrap().to_num::<u64>(), amount_b)
        } else {
            (amount_a, I64F64::from_num(amount_a).checked_mul(ratio).unwrap().to_num::<u64>())
        }
    };
